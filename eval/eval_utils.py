...
def get_hausdorff_distance(
    ...
    """Calculate Hausdorff distance between ground truth and predicted points"""
    gt_num_points = gt_points.shape[0]
    pred_num_points = pred_points.shape[0]
    ...
    distances = (points_gt_matrix - points_pred_matrix).pow(2).sum(dim=-1).sqrt()
    
    # Directed Hausdorff: max of min distances from pred to gt
    min_dist_pred_to_gt = distances.min(dim=0)[0]
    hausdorff_pred_to_gt = min_dist_pred_to_gt.max()
    
    # Directed Hausdorff: max of min distances from gt to pred
    min_dist_gt_to_pred = distances.min(dim=1)[0]
    hausdorff_gt_to_pred = min_dist_gt_to_pred.max()
...
def get_cd_nc_for_points(
    ...
    cd, nc = get_chamfer_distance_and_normal_consistency(
...
def evaluate(
    ...
    with tqdm.trange(len(eval_list)) as pbar:
        for idx in pbar:
            ...
            points_cd, points_nc, points_hd = get_cd_nc_for_points(
            ...
            mean_metrics["hausdorff_distance"] += points_hd
            
            if points_cd>0 and points_nc>0 and points_hd>0:
                total_entries += 1
