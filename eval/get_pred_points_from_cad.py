...
def main(args):
    ...
    for idx in tqdm(range(len(eval_list))):
        ...
        base_path = config['pred_meshs_folder_to'] + object_name
        
        if os.path.exists(base_path + '.obj'):
            gt_file = base_path + '.obj'
        elif os.path.exists(base_path + '.stl'):
            gt_file = base_path + '.stl'
        else:
            print(f"Warning: No .obj or .stl file found for {object_name}")
            continue

        mesh = trimesh.load(gt_file, force='mesh')
        ...
        resmpl_edge_points = get_edge_points(to_tensor(points), to_tensor(normals),0.9)
        write_ply_point_normal(
...
if __name__ == "__main__":
	...
	arg_parser = argparse.ArgumentParser(description="eval")
